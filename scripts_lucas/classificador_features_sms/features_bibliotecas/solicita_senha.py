...
Exemplo positivo: Para confirmar seu cadastro, informe sua senha de acesso no formulário abaixo.
Contraexemplo: Nunca compartilhe sua senha.
"""

HIPOTESE = 'O remetente pede que o destinatário informe, envie ou digite sua senha.'
EXPLICACAO = 'Detecta pedido de senha; menção a senha alterada e conselho para nunca compartilhar são controles negativos.'


def medir(sms: str) -> dict:
    """Retorna escore, limiar e hipótese usados na decisão."""
    from .classificador import obter_classificador
    return obter_classificador().medir_feature('solicita_senha', sms)
