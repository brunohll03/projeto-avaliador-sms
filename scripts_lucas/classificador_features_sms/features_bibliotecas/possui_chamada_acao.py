...
Exemplo positivo: Informe seu CPF e clique no botão abaixo para confirmar.
Contraexemplo: Nunca compartilhe sua senha.
"""

HIPOTESE = 'O remetente solicita que o destinatário realize uma ação concreta.'
EXPLICACAO = 'Categoria ampla que pode coexistir com clique, pagamento e envio de dados. Relato no passado e proibição não são pedidos afirmativos.'


def medir(sms: str) -> dict:
    """Retorna escore, limiar e hipótese usados na decisão."""
    from .classificador import obter_classificador
    return obter_classificador().medir_feature('possui_chamada_acao', sms)
