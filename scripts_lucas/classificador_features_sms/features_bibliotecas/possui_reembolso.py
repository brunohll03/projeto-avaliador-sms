...
Exemplo positivo: Identificamos um reembolso de R$ 350,00 disponível para você. Clique aqui para receber.
Contraexemplo: O reembolso foi negado.
"""

HIPOTESE = 'A mensagem informa que o destinatário tem reembolso, restituição ou devolução de dinheiro disponível para receber.'
EXPLICACAO = 'Política geral de reembolso ou pedido negado não equivale a valor disponível.'


def medir(sms: str) -> dict:
    """Retorna escore, limiar e hipótese usados na decisão."""
    from .classificador import obter_classificador
    return obter_classificador().medir_feature('possui_reembolso', sms)
